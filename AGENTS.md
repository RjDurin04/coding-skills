...
4. For engineering tasks, apply the generated fast-path screen in `GEMINI.md`.
   Use its precompiled baseline only when every exclusion is demonstrably
   false. If an exclusion is true or unknown, apply
   `rules/governance-router.md`. Use `governance-manifest.json` as the
   authoritative routing/inventory source and apply the union of triggered
   `rules/*.md`.
...
9. Install the pinned tooling in `requirements-governance.txt` in an isolated
   ...
   rules pack. The deterministic checks cover executable JSON Schema
   validation, generated-governance parity, manifest/schema/inventory
   integrity, exact composition and independent semantic routing coverage,
   scorer bypasses and exit contracts, the offline model-runner protocol,
   capability coverage/scoring, and validator mutation tests. These checks do
   not establish live-model behavior or production readiness.
