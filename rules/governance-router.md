...
| `algorithmic_efficiency` | `standard` | `none` | `algorithmic-efficiency-gate` | `performance-engineer` | none | A hot path, query, loop, transform, cache, queue, memory or allocation pressure, throughput or tail-latency target, or user-sized and growing workload needs complexity or performance analysis. |
| `api_contract` | `structural` | `none` | `testing-strategy`, `security-and-privacy-gate`, `complexity-budget`, `evolutionary-stewardship` | `api-and-contract-engineer` | `drift-guardian` | A public API, event, webhook, WebSocket/SSE/long-poll stream, HTTP/RPC/gRPC protocol, schema, SDK-facing surface, versioning policy, or compatibility contract changes. |
...
| `database_design` | `structural` | `none` | `data-integrity-and-migrations` | `data-and-database-engineer` | none | A data model, query shape, index, transaction, isolation, optimistic or pessimistic locking, ownership, or database architecture decision changes. |
...
| `distributed_system` | `structural` | `none` | `testing-strategy`, `observability-by-design`, `operational-resilience` | `distributed-systems-engineer` | `adversarial-test-forge` | Cross-process, cross-service, queue or Pub/Sub, distributed transaction, region, ordering, delivery, consistency, retry, coordination, or partition behavior changes. |
...
| `runtime_concurrency_or_resource_lifecycle` | `standard` | `none` | `runtime-resource-safety`, `testing-strategy` | `runtime-and-concurrency-engineer` | `performance-engineer`, `adversarial-test-forge` | Process-local threads, async tasks, event loops, synchronization, cancellation, pools, blocking behavior, memory leaks, garbage collection, handles, sockets, timers, or shutdown/resource lifecycle changes. |
...
| `security_relevant_configuration` | `structural` | `none` | `configuration-and-feature-flags`, `security-and-privacy-gate` | `security-reviewer` | `drift-guardian` | Configuration materially changes authentication, authorization, OAuth/OIDC or JWT validation, secrets, TLS, origin/proxy/network trust, WAF or edge security, security headers, execution permissions, or data-exposure controls. |
...
| `supply_chain_change` | `standard` | `none` | `supply-chain-and-build-integrity`, `security-and-privacy-gate` | none | `security-reviewer` | A package version or range, resolver/lockfile, package source, build or dependency cache, build script, CI action, generated artifact, or dependency trust decision changes. |
...
| `user_interface_accessibility` | `standard` | `none` | `interface-and-accessibility-gate` | `accessible-interface-engineer` | none | An interface, custom control, focus/navigation flow, assistive-technology support, or accessibility defect needs specialist inclusive-design engineering or compliance verification. |
| `user_interface_design_system` | `structural` | `none` | `interface-and-accessibility-gate`, `complexity-budget`, `evolutionary-stewardship` | `design-system-engineer` | none | Reusable UI components, semantic design tokens, component contracts, cross-surface design systems, token architecture, or design system governance and versioning change. |
| `user_interface_frontend_implementation` | `standard` | `none` | `interface-and-accessibility-gate`, `testing-strategy` | `frontend-interface-engineer` | none | A resolved user interface design, screen, or component is implemented in frontend code with complete runtime states, responsiveness, and framework conventions. |
| `user_interface_interaction_motion` | `standard` | `none` | `interface-and-accessibility-gate` | `interaction-and-motion-designer` | none | Micro-interactions, gestures, transitions, animations, spatial metaphors, layout choreography, physics, or motion hierarchy and reduced-motion behavior are designed or implemented. |
| `user_interface_quality_review` | `standard` | `none` | `interface-and-accessibility-gate` | `interface-quality-reviewer` | none | An existing user interface is reviewed, audited, or evaluated for UX, visual hierarchy, responsiveness, motion, accessibility, content integrity, or implementation evidence without making changes. |
| `user_interface_visual_design` | `standard` | `none` | `interface-and-accessibility-gate` | `visual-design-specialist` | none | The primary unresolved decision is an interface's visual language: art direction, typography, hierarchy, composition, color, density, iconography, theming, or brand expression. |
